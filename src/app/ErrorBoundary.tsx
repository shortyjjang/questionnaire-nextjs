...
class ErrorBoundary extends React.Component<Props, State> {
  ...
  categorizeError(error: Error): string {
    ...
    if (error.message.includes('prop') || error.message.includes('undefined')) return 'props';
    return 'general';
  }
    ...
    if (this.state.hasError) {
      return (
        <div className="bg-red-50 border border-red-200 p-4 rounded-md">
          <h2 className="text-red-700 text-lg font-medium">문제가 발생했습니다</h2>
          <p className="text-red-500">컴포넌트 렌더링 중 오류가 발생했습니다.</p>
          <button
